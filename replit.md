...
-   **AI Campaign Advisor with Conversation Memory**: Intelligent chatbot with persistent conversation history stored in database. The AI automatically remembers all previous interactions with each user, maintaining full context across sessions without displaying history in the UI. Uses OpenAI API with conversation tracking via `ai_conversations` and `ai_messages` tables.
    -   **AI Security Layer**: Comprehensive security protection including system prompt restrictions (prevents direct DB access), PII redaction middleware (scans and redacts phone/email/addresses from AI responses), audit logging (tracks all AI interactions with security alerts), rate limiting (30 requests/hour per user), and admin security monitoring dashboard to track suspicious activity.
