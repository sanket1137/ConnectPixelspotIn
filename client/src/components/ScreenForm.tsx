...
// Admin schema includes ownerId selector
const adminScreenSchema = baseScreenSchema.extend({
  ownerId: z.string().min(1, "Screen owner is required"),
});

type BaseScreenForm = z.infer<typeof baseScreenSchema>;
type AdminScreenForm = z.infer<typeof adminScreenSchema>;
...
export default function ScreenForm({ isAdminMode, currentUserId, onSuccess }: ScreenFormProps) {
  const [, setLocation] = useLocation();
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [uploadedImageURL, setUploadedImageURL] = useState<string | null>(null);

  const form = useForm<AdminScreenForm | BaseScreenForm>({
    resolver: zodResolver(isAdminMode ? adminScreenSchema : baseScreenSchema),
    defaultValues: {
      ...(isAdminMode && { ownerId: "" }),
      name: "",
      category: "",
  ...
  // Fetch all screen owners (only for admin mode)
  ...
  const createScreenMutation = useMutation({
    mutationFn: async (data: AdminScreenForm | BaseScreenForm) => {
      const screenData = {
        ...data,
        // For owner mode, use currentUserId; for admin mode, use form ownerId
        ownerId: isAdminMode ? (data as AdminScreenForm).ownerId : currentUserId,
      ...
      const endpoint = isAdminMode ? "/api/admin/screens/create" : "/api/owner/screens";
      return apiRequest("POST", endpoint, screenData);
    },
    onSuccess: () => {
      toast({
        title: "Screen Added",
        description: `Screen has been successfully added${isAdminMode ? ' for the owner' : ''}.`,
      });
      queryClient.invalidateQueries({ queryKey: isAdminMode ? ["/api/admin/screens"] : ["/api/owner/screens"] });
      
      if (onSuccess) {
        onSuccess();
      } else {
        setLocation(isAdminMode ? "/admin/screens" : "/owner/screens");
      }
    },
    onError: () => {
      toast({
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          {/* Screen Owner Selection (Admin Only) */}
          {isAdminMode && (
            <Card>
              <CardHeader>
